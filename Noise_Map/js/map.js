...
// LayerGroup that holds all event circle markers
let _markersGroup;
...
function initMap() {
  ...
  // Layer group for all event markers (add to map now so markers appear on top)
  _markersGroup = L.layerGroup().addTo(map);

  // Build the map legend from the colorMap in config.js
  buildLegend();
...
/**
 * Builds a Leaflet control legend in the bottom-right corner.
 * Reads categories and colours from CONFIG.colorMap.
 * Also adds an entry for the default colour.
 */
  ...
  legend.onAdd = function () {
    ...
    div.innerHTML = `
      <div class="map-legend__title">
        ${CONFIG.columns.colorBy.replace(/_/g, ' ')}
      </div>
    `;

    // One row per colour entry
    const entries = Object.entries(CONFIG.colorMap.values);
    entries.forEach(([label, colour]) => {
      div.innerHTML += `
        <div class="map-legend__item">
          <span class="map-legend__swatch" style="background:${colour};"></span>
          <span class="map-legend__label">${escapeHtml(label)}</span>
        </div>
      `;
    });
    ...
    div.innerHTML += `
      <div class="map-legend__item">
        <span class="map-legend__swatch" style="background:${CONFIG.colorMap.default};"></span>
