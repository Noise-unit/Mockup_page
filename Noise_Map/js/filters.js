...
function applyFilters() {
  ...
  _markerRegistry.forEach(({ record, marker }) => {
    ...
    // Show or hide by adjusting opacity (keeps markers in the DOM for performance)
    marker.setStyle(
      pass
        ? { opacity: 1,   fillOpacity: CONFIG.marker.fillOpacity }
...
function resetFilters() {
  ...
  _markerRegistry.forEach(({ marker }) =>
    marker.setStyle({ opacity: 1, fillOpacity: CONFIG.marker.fillOpacity })
  );
